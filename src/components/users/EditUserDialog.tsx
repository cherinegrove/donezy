...
export function EditUserDialog({ user, isOpen, onClose }: EditUserDialogProps) {
  const { addUser, updateUser, teams } = useAppContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && user) {
        updateUser(user.auth_user_id, { name, email, jobTitle, phone, status, teamIds: selectedTeamIds });
      ...
      toast({ title: "Error", description: "There was a problem saving the user", variant: "destructive" });
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Doe" required autoComplete="off" />
            </div>
            <div>
              <Label htmlFor="email">Email *</Label>
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="john@company.com" required autoComplete="off" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input id="jobTitle" value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder="Senior Developer" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {availableTeams.map((team) => (
                <div key={team.id} className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor={`team-${team.id}`} className="text-sm font-normal cursor-pointer">
                    {team.name}
                  </Label>
                </div>
              ))}
