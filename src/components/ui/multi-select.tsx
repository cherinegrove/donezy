...
function normalizeOptions(rawOptions: any): Option[] {
  if (!Array.isArray(rawOptions)) {
    console.warn("MultiSelect: options is not an array", rawOptions);
    return [];
  }
...
// Utility function to normalize selected values safely
function normalizeSelectedValues(rawValues: any): string[] {
  if (!Array.isArray(rawValues)) {
    console.warn("MultiSelect: selectedValues is not an array", rawValues);
    return [];
  }
  ...
  if (!onValueChange || typeof onValueChange !== 'function') {
    ...
    return (
      <div className={cn("min-h-10 h-auto w-full justify-between border border-input bg-background px-3 py-2 text-sm ring-offset-background rounded-md", className)}>
        <span className="text-muted-foreground">Invalid configuration</span>
      </div>
    );
  }
