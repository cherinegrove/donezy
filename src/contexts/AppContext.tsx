...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      if (sessionUserDb) {
        ...
        // Primary match by auth_user_id failed — fallback to email match
        console.warn(
          "Session user not found in database users map by auth_user_id. Trying email fallback.",
        );
        ...
        if (!finalUser) {
          // Last resort: create a stub user from session data
          console.warn(
            "User missing from DB entirely. Creating stub from session.",
            session.user.id,
          );
          finalUser = {
            id: session.user.id,
            ...
            roleId: "user",
            systemRoles: [],
            rbacRoles: rbacRolesMap[session.user.id] || [],
          } as User;
        }

        // Merge in any roles keyed directly by the session auth ID
        ...
        // Patch auth_user_id to ensure writes use the true session ID
  ...
  const deleteProject = async (
    ...
    if (!hasPermission(currentUserRef.current, "projects", "delete", "own")) {
      return {
        success: false,
  ...
  const addTask = async (
    ...
    try {
      ...
      if (data) {
        ...
        if (data.assignee_id) {
          const projectName = projectsRef.current.find(
          ...
          supabase.functions
            ...
            .then(({ error }) => {
              if (error) {
                console.error(
                  "Error dispatching task_assigned notification:",
                  error,
                );
              }
            });
  ...
  const updateTask = async (
    ...
    try {
      ...
      if (
        ...
        changes.push(
          `changed status from "${statusLabel(currentTask.status)}" to "${statusLabel(updates.status)}"`,
        );
      }
      if (
      ...
      const notifyProjectName = currentTask.projectId
        ? projectsRef.current.find((p) => p.id === currentTask.projectId)
            ?.name
        : undefined;
        ...
        supabase.functions
          .invoke("dispatch-notification", {
            body: {
              ...
              actorId: session.user.id,
              taskId,
              excludeRecipientIds: assigneeChanged
                ? [updates.assigneeId]
                : [],
              context: {
                taskTitle: notifyTaskTitle,
          ...
          .then(({ error }) => {
            if (error) {
              console.error(
                "Error dispatching task_status_changed notification:",
                error,
              );
            }
          });
      ...
      if (
        ...
        supabase.functions
          .invoke("dispatch-notification", {
            body: {
              ...
              context: {
                ...
                actorName: currentUserRef.current?.name,
                projectName: notifyProjectName,
                changesSummary: Object.entries(changesSummary)
          ...
          .then(({ error }) => {
            if (error) {
              console.error(
                "Error dispatching task_updated notification:",
                error,
              );
            }
          });
  ...
  const reorderTasks = async (
    ...
    try {
      ...
      if (
        ...
        const reorderProjectName = projectsRef.current.find(
  ...
  const addComment = async (
    ...
    try {
      ...
      const commentProjectName = taskData?.project_id
        ? projectsRef.current.find((p) => p.id === taskData.project_id)?.name
        : undefined;
      ...
      supabase.functions
        ...
        .then(({ error }) => {
          if (error) {
            console.error(
              "Error dispatching task_commented notification:",
              error,
            );
          }
        });
      ...
      if (mentionedUserIds && mentionedUserIds.length > 0) {
        supabase.functions
          ...
          .then(({ error }) => {
            if (error) {
              console.error(
                "Error dispatching mentioned notification:",
                error,
              );
            }
          });
