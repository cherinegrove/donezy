  ...
  SELECT jsonb_build_object(
    ...
    'approved_hours', (
      ...
      WHERE t.project_id = v_project_id
        AND te.task_id IS NOT NULL
        AND te.task_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        AND te.approval_status IN ('approved', 'approved_billable', 'approved_non_billable')
    ),
    'declined_hours', (
      SELECT COALESCE(SUM(te.duration) / 60.0, 0)
      FROM public.time_entries te
      ...
      WHERE t.project_id = v_project_id
        AND te.task_id IS NOT NULL
        AND te.task_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        AND te.approval_status = 'declined'
    ),
    'pending_hours', (
      SELECT COALESCE(SUM(te.duration) / 60.0, 0)
      FROM public.time_entries te
      ...
      WHERE t.project_id = v_project_id
        AND te.task_id IS NOT NULL
        AND te.task_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        AND (te.approval_status IS NULL OR te.approval_status = 'pending')
    ),
    'comments', (
      SELECT jsonb_agg(jsonb_build_object(
        'id', pc.id,
