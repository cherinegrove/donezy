...
-- Handle all dependencies first

-- Remove any users with null auth_user_id (these are orphaned records)
DELETE FROM public.users WHERE auth_user_id IS NULL;

-- Update the policy that references users.id to use auth_user_id instead
DROP POLICY IF EXISTS secure_message_creation ON public.messages;

CREATE POLICY "secure_message_creation" ON public.messages
FOR INSERT 
WITH CHECK (
  ...
  EXISTS (
    SELECT 1 
    FROM users 
    WHERE users.auth_user_id::text = messages.from_user_id AND 
